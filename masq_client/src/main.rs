...
struct Args {
    ...
    /// the congestion control for quic connection,consistent with the server. default cubic. (设定quic流控,可选: reno,bbr,cubic)
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut socks_client = Socks5Client::new();
    socks_client.bind(&op.socks5).await?;
    socks_client.run(&op.connect, op.skip_verify, &op.ca, &op.congestion_control).await
    // Ok(())
}